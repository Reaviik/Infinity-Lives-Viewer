...
const twitchChannels = {
  ...
  sauletagames:     { name: 'Sauletagames',    color: 'bg-gradient-to-br from-purple-500 to-indigo-500', type: 'plus' },
  lorran_aa:        { name: 'SabugOza',        color: 'bg-gradient-to-br from-purple-500 to-indigo-500', type: 'plus' },
  raptornoed:       { name: 'Raptornoed',      color: 'bg-gradient-to-br from-blue-500 to-teal-500', type: 'normal' },
  laluts1:          { name: 'LALUTS',          color: 'bg-gradient-to-br from-blue-500 to-teal-500', type: 'normal' },
  ...
  balborock:        { name: 'BalboRock',       color: 'bg-gradient-to-br from-emerald-500 to-lime-500', type: 'iniciante' },
  bhaaskara:        { name: 'Bhaaskara',       color: 'bg-gradient-to-br from-gray-400 to-gray-600', type: 'desertor' },
  barbasirius:      { name: 'Obarbasirius',    color: 'bg-gradient-to-br from-gray-400 to-gray-600', type: 'desertor' },
  lordrebechi:      { name: 'LordRebechi',     color: 'bg-gradient-to-br from-gray-400 to-gray-600', type: 'desertor' },
  ofirofiro:        { name: 'OfirOfiro',       color: 'bg-gradient-to-br from-gray-400 to-gray-600', type: 'desertor' },
  srpikenno:        { name: 'Pikenno',         color: 'bg-gradient-to-br from-gray-400 to-gray-600', type: 'desertor' },
...
function getChannelButtonClasses(channelConfig) {
  const baseClasses = 'flex items-center space-x-2 mt-1 w-full hover:bg-gray-700 rounded-lg px-3 py-1.5 transition-all duration-300 relative min-h-[50px] backdrop-blur-sm';
  let typeClasses = '';
...
function showOnlineNowCard(channel) {
  ...
  notificationCard.innerHTML = `
    ...
    <div id="notification-status-${channel}" class="absolute bottom-0 right-0 w-3 h-3 bg-green-400 rounded-lg border-2 border-gray-850 animate-pulse"></div>
  `;
  ...
  if (avatarUrl) {
    ...
    if (notificationAvatarDiv) {
      notificationAvatarDiv.innerHTML = `<img src="${avatarUrl}" alt="avatar" class="w-8 h-8 rounded-lg object-cover border-2 border-green-400" />`;
...
function updateStatusIndicator(channel, isOnline) {
  ...
  if (statusElement) {
    if (isOnline) {
      statusElement.className = 'absolute bottom-0 right-0 w-3 h-3 bg-red-500 rounded-lg border-2 border-gray-850 animate-pulse';
    } else {
      statusElement.className = 'absolute bottom-0 right-0 w-3 h-3 bg-gray-500 rounded-lg border-2 border-gray-850';
    }
  }
  ...
  openLives.forEach(channel => {
    ...
    liveDiv.innerHTML = `
      <div class="flex flex-row gap-2 absolute top-1 right-2 z-20">
        <button onclick="closeLive('${channel}')" title="Fechar" class="live-action-btn w-4 h-4 rounded-lg text-white font-semibold text-base flex items-center justify-center">×</button>
        <button onclick="window.favoritarCanal && window.favoritarCanal('${channel}');this.classList.toggle('text-yellow-400');this.classList.toggle('text-gray-400');" title="Favoritar" class="live-action-btn w-4 h-4 rounded-lg font-semibold text-base flex items-center justify-center ${isFav ? 'text-yellow-400' : 'text-gray-400'}">★</button>
      </div>
...
function updateChatButtons() {
  ...
  openLives.forEach(async (channel, index) => {
    ...
    button.className = 'flex items-center justify-center w-10 h-10 rounded-lg hover:bg-gray-700 transition-all duration-200 p-1';
    ...
    button.innerHTML = `
      <div class="w-8 h-8 bg-gray-600 rounded-lg flex items-center justify-center font-bold border-2 border-transparent transition-all duration-200" id="chat-avatar-${channel}">${firstLetter}</div>
    `;
    ...
    if (avatarUrl) {
      ...
      if (avatarDiv) {
        avatarDiv.innerHTML = `<img src="${avatarUrl}" alt="avatar" style="width: 32px; height: 32px; border-radius: 8px; object-fit: cover; border: 2px solid;" />`;
...
async function updateAllAvatars() {
  for (const channel of Object.keys(twitchChannels)) {
    ...
    if (avatarDiv && avatarUrl) {
      avatarDiv.innerHTML = `<img src="${avatarUrl}" alt="avatar" class="w-8 h-8 rounded-lg object-cover border-2" />`;
      avatarDiv.style.background = 'none';
      avatarDiv.style.color = 'transparent';
...
async function updateAllChannelsListSmooth() {
  ...
  function createOrUpdateChannelButton(channel, isOnline) {
    ...
    // Adicionar o título da live no tooltip
    const liveTitle = isOnline && userData.stream && userData.stream.title
      ? userData.stream.title
      : 'Offline';
    if (!button) {
      ...
      `;
      // Adiciona o tooltip do título da live
      button.setAttribute('data-tooltip', isOnline ? `🔴 ${liveTitle}` : 'Offline');
      container.appendChild(button);
    } else {
      // Atualiza o tooltip do botão existente
      button.setAttribute('data-tooltip', isOnline ? `🔴 ${liveTitle}` : 'Offline');
    }
    // Avatar
    const avatarDiv = document.getElementById(`avatar-${channel}`);
    ...
    if (statusDiv) {
      statusDiv.className = `canal-action-btn ${isOnline ? 'bg-red-500 animate-pulse' : 'bg-gray-500'} rounded-lg border-2 border-gray-850`;
...
// (Removido: agora está em stats.js)
// ... existing code ...
